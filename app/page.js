...
const PortfolioPack = () => {
  ...
  const cards = [
    ...
    },
    {
      ...
      company: "Mobalytics",
      year: "Summer 2023",
      image: "/images/mobalytics.gg.webp",
      description:
  ...
  const handleCardSwipe = (_, info) => {
    if (Math.abs(info.offset.x) > 100) {
      if (currentCardIndex < cards.length - 1) {
        ...
        // Delay the sound to sync with animation
        setTimeout(() => {
          playCardSwipeSound();
        }, 800); // Adjust this to sync perfectly — try 300-400ms
      } else {
